...
- Human corrections and contextualisations after Stage 3a.
- Reporting rule: preserve the substantive sequence of the LLM output where useful, quote short verbatim excerpts only when analytically relevant, and summarise the rest without turning results into discussion.
...
- A frequent area is not a gap.
- A candidate direction must identify the theoretical anchor that supports it.
- Candidate directions should build from the theoretical anchors and tensions mapped in Stage 3a, not from raw internal citation or close-coauthorship rankings.
- Separate substantive directions from comments about the protocol itself.
- Treat emerging technologies as candidate directions only when there is enough evidence or a clearly justified exploratory reason.
- Mark when a suggestion is merely a new label for work the team already does.
